<template>
  <div class="content">
    ...
    <div class="feeds" v-if="data.feeds.length > 1">
      <Feed
        :title="data.popular_feed.title"
        :url="data.popular_feed.url"
        :mediaType="data.popular_feed.mediaType"
      ></Feed>
      <Feed
        v-for="feed of data.feeds"
        :url="feed.url"
        :title="feed.title"
        :mediaType="feed.mediaType"
        :key="data.feeds.indexOf(feed)"
      ></Feed>
    </div>
  </div>
</template>
  ...
  setup() {
    ...
    function buildPopularFeed() {
      ...
      let obj = {
        ...
        url:
          data.url.base +
          data.api_params[r_num] +
          "/popular" +
          data.url.defaultParams,
